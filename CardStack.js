...
class CardStack extends Component {
  ...
  render() {
    ...
    const likeOpacity = drag.x.interpolate({
      inputRange: [0, width / 4],
      outputRange: [0, 1],
    });
    const nopeOpacity = drag.x.interpolate({
      inputRange: [-width / 4, 0],
      outputRange: [1, 0],
    });
    
    const HotLabel = this.props.hotLabel;
    const NotLabel = this.props.notLabel;
    return (
      <View {...this._panResponder.panHandlers} style={[{position: 'relative'}, this.props.style]}>
        ...
        <Animated.View
          ...
          >
            {this.props.notLabel && (
              <NotLabel nopeOpacity={(topCard === 'cardB') ? nopeOpacity : 0} />
            )}
            {this.props.hotLabel && (
              <HotLabel likeOpacity={(topCard === 'cardB') ? likeOpacity : 0} />
            )}
          </View>
        </Animated.View>
        <Animated.View
          ...
          >
            {this.props.notLabel && (
              <NotLabel nopeOpacity={(topCard === 'cardA') ? nopeOpacity : 0} />
            )}
            {this.props.hotLabel && (
              <HotLabel likeOpacity={(topCard === 'cardA') ? likeOpacity : 0} />
            )}
          </View>
        </Animated.View>
      </View>
...
CardStack.propTypes = {
  hotLabel: PropTypes.object,
  notLabel: PropTypes.object,
